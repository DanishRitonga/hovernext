...
def validation(
    ...
    val_new = torch.mean(torch.stack(val_loss))
    print("Step: ", step)
    if metric == "lizard":
        df, _ = calc_MPQ(pred_list, gt_list, nclasses)
        pq_p = torch.tensor(df["pq"][0]).to(device)
        mpq_p = torch.tensor(df["multi_pq+"][0]).to(device)
        print("PQ:", pq_p)
        print("mPQ+", mpq_p)
        ...
        pan_mpq = torch.tensor(pan_mpq).to(device)
        ...
        f1_c = torch.from_numpy(f1).to(device)
        hd_c = torch.from_numpy(hd).to(device)
        print("F1:", f1_c)
        print("HD:", hd_c)
        print("Validation loss: ", val_new)
